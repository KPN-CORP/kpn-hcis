    ...
    WithStyles
{
    ...
    public function map($businessTrip): array
    {
        ...
        // $relatedCA = $this->caData->firstWhere(
        //     "no_sppd",
        ...
        // $declarationAt = $relatedCA
        //     ? Carbon::parse($relatedCA->declaration_at)->format("d-m-Y")
        //     : "-";
        ...
        // } elseif (in_array($status, $pendingDecStatuses)) {
        ...
        // $adminStatuses = ["Doc Accepted", "Return/Refund", "Verified"];

        // if (in_array($status, $adminStatuses)) {
        //     $displayAdminStatus = $status;
        //     if ($displayAdminStatus == "Doc Accepted") {
        //         $dateAdminStatus = Carbon::parse($businessTrip->doc_at)->format(
        //             "d-m-Y",
        //         );
        //     } elseif ($displayAdminStatus == "Return/Refund") {
        //         $dateAdminStatus = Carbon::parse(
        //             $businessTrip->return_at,
        //         )->format("d-m-Y");
        //     } elseif ($displayAdminStatus == "Verified") {
        //         $dateAdminStatus = Carbon::parse(
        //             $businessTrip->verified_at,
        //         )->format("d-m-Y");
        //     }
        // } else {
        //     $displayAdminStatus = "-";
        ...
        // $relatedBtApproval = $this->btApprovalData->firstWhere(
        //     "bt_id",
        //     $businessTrip->id,
        // );
        // $doc_at = $businessTrip->doc_at
        //     ? Carbon::parse($businessTrip->doc_at)->format("d-m-Y")
        //     : "-";
        // $return_at = $businessTrip->return_at
        //     ? Carbon::parse($businessTrip->return_at)->format("d-m-Y")
        //     : "-";
        // $verified_at = $businessTrip->verified_at
        //     ? Carbon::parse($businessTrip->verified_at)->format("d-m-Y")
        //     : "-";
        ...
        $totalCA =
            $relatedCAs && $relatedCAs->count() > 0
                ? $relatedCAs->sum("total_ca")
                : 0;
        $totalReal =
            $relatedCAs && $relatedCAs->count() > 0
                ? $relatedCAs->sum("total_real")
                : 0;
        $totalCost =
            $relatedCAs && $relatedCAs->count() > 0
                ? $relatedCAs->sum("total_cost")
                : 0;
        $declarationAt = $relatedCAFirst
            ? Carbon::parse($relatedCAFirst->declaration_at)->format("d-m-Y")
            : "-";
        $noCA =
            $relatedCAs->count() > 0
                ? $relatedCAs->pluck("no_ca")->implode(", ")
                : "-";
        ...
        $displayDecStatus = $relatedCAFirst ? $displayDecStatus : "-";
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet
            ->getStyle("I2:I" . $sheet->getHighestRow())
            ->applyFromArray($currencyStyle); // For total_ca
        $sheet
            ->getStyle("J2:J" . $sheet->getHighestRow())
            ->applyFromArray($currencyStyle); // For total_real
        $sheet
            ->getStyle("K2:K" . $sheet->getHighestRow())
