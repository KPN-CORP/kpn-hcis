...
class MedicalFailedImportExport implements
    FromArray,
    WithHeadings,
    WithStyles,
    WithEvents
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $note =
                    "Silahkan Upload file ini saja setelah revisi, karena data sebelumnya telah berhasil, dan hapus note ini.";
                $noteRow = $highestRow + 2; // Menambahkan catatan dua baris di bawah data
                $sheet->setCellValue("A" . $noteRow, $note);
                $sheet->mergeCells(
                    "A" . $noteRow . ":" . $highestColumn . $noteRow,
                ); // Menggabungkan sel untuk catatan
                $sheet
                    ->getStyle("A" . $noteRow)
                    ->getAlignment()
                    ->setHorizontal(
                        \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER,
                    ); // Memusatkan teks catatan
            },
        ];
    }
}
