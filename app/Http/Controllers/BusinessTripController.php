...
class BusinessTripController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->groupCompanies = Location::select("company_name")
            ->when(!empty($groupCompanyCodes), function ($query) use (
                $groupCompanyCodes,
            ) {
        ...
        $this->locations = Location::select("company_name", "area", "work_area")
            ->when(
                !empty($workAreaCodes) || !empty($groupCompanyCodes),
                function ($query) use ($workAreaCodes, $groupCompanyCodes) {
        ...
        $this->companies = Company::select(
            ...
            ->when(!empty($companyCodes), function ($query) use (
                $companyCodes,
            ) {
    ...
    {
        $user = Auth::user();
        $query = BusinessTrip::where("user_id", $user->id)->orderBy(
            "created_at",
            "desc",
        );
        ...
        $caTransactions = ca_transaction::whereIn("no_sppd", $sppdNos)
            ...
            ->get()
            ->groupBy("no_sppd");
        $tickets = Tiket::whereIn("no_sppd", $sppdNos)
            ->get()
            ->groupBy("no_sppd");
        ...
        // Get manager names
        $managerL1Names = Employee::whereIn(
            "employee_id",
            $sppd->pluck("manager_l1_id"),
        )->pluck("fullname", "employee_id");
        $managerL2Names = Employee::whereIn(
            "employee_id",
            $sppd->pluck("manager_l2_id"),
        )->pluck("fullname", "employee_id");
        ...
        return view(
            "hcis.reimbursements.businessTrip.businessTrip",
            compact(
                "sppd",
                "parentLink",
                "link",
    ...
    public function delete($id)
    {
        ...
        return redirect()
            ...
            ->with(
                "success",
                "Business Travel marked as deleted along with related records.",
            );
    }
    ...
    public function deleteAdmin($id)
    {
        ...
        return redirect()
            ->back()
            ->with("success", "Business Travel marked as deleted.");
    }
    ...
    public function formUpdate($id)
    {
        ...
        $group_company = Employee::where("id", $userId)
            ->pluck("group_company")
            ->first();
        $bt_sppd = BusinessTrip::where("status", "!=", "Done")
        ...
        $perdiem = ListPerdiem::where("grade", $employee_data->job_level)
            ...
            ->first();
        $job_level = Employee::where("id", $userId)
            ->pluck("job_level")
            ->first();
    ...
    public function update(Request $request, $id)
    {
        ...
        if (
            $request->tujuan === "Others" &&
            !empty($request->others_location)
        ) {
        ...
        if (
            $employee->group_company == "Plantations" ||
            $employee->group_company == "KPN Plantations"
        ) {
            $allowance = "Perdiem";
        } else {
            $allowance = "Allowance";
        ...
        if ($vtDalam === "Ya") {
            ...
            $existingTaksi = Taksi::where("no_sppd", $oldNoSppd)
                ->get()
                ->keyBy("id");
            ...
            } elseif ($request->jns_dinas === "luar kota") {
        ...
        if ($messDalam === "Ya") {
            ...
            $existingMesses = Mess::where("no_sppd", $oldNoSppd)
                ->get()
                ->keyBy("id");
            $newNoMess = null;
                ...
                if (!empty($value)) {
                    ...
                    if ($messId && isset($existingMesses[$messId])) {
                        ...
                        $mess->update([
                            ...
                            "tgl_masuk_mess" =>
                                $messData["tgl_masuk_mess"][$key],
                            "tgl_keluar_mess" =>
                                $messData["tgl_keluar_mess"][$key],
                            "total_hari_mess" =>
                                $messData["total_hari_mess"][$key],
                            "approval_status" => $statusValue,
                            "contribution_level_code" =>
            ...
            Mess::where("no_sppd", $oldNoSppd)
                ->whereNotIn("id", $processedMessIds)
                ->delete();
        } else {
        ...
        if ($htlDalam === "Ya") {
            // Get all existing hotels for this business trip
            $existingHotels = Hotel::where("no_sppd", $oldNoSppd)
                ->get()
                ->keyBy("id");
            $newNoHtl = null;
        ...
        if ($tktDalam === "Ya") {
            ...
            $existingTickets = Tiket::where("no_sppd", $oldNoSppd)
                ->get()
                ->keyBy("noktp_tkt");
            ...
            foreach ($ticketFields["noktp_tkt"] as $key => $value) {
                if (!empty($value)) {
                    ...
                    $ticketData = [
                        ...
                        "tgl_brkt_tkt" =>
                            $ticketFields["tgl_brkt_tkt"][$key] ?? null,
                        "jam_brkt_tkt" =>
                            $ticketFields["jam_brkt_tkt"][$key] ?? null,
                        "jenis_tkt" => $ticketFields["jenis_tkt"][$key] ?? null,
                        "type_tkt" => $ticketFields["type_tkt"][$key] ?? null,
                        "tgl_plg_tkt" =>
                            $ticketFields["tgl_plg_tkt"][$key] ?? null,
                        "jam_plg_tkt" =>
                            $ticketFields["jam_plg_tkt"][$key] ?? null,
                    ...
                    if (!$employee_data) {
                        return redirect()
                            ->back()
                            ->with("error", "NIK $value not found");
                    }
        ...
        if ($request->ca === "Ya" || $request->ca_dalam === "Ya") {
            $businessTripStatus = $request->input("status");
            $ca = CATransaction::where("no_sppd", $oldNoSppd)
                ->where("type_ca", "dns")
                ->first();
            if (!$ca) {
            ...
            if ($request->jns_dinas === "dalam kota") {
                $ca->date_required = \Carbon\Carbon::parse($ca->start_date)
                    ->subDay()
                    ->format("Y-m-d");
            } else {
                $ca->date_required = $request->date_required_2;
            ...
            $ca->total_days = Carbon::parse($request->mulai)->diffInDays(
                Carbon::parse($request->kembali),
            );
            ...
            if ($request->has("start_bt_perdiem")) {
                foreach ($request->start_bt_perdiem as $key => $startDate) {
                    ...
                    $other_location =
                        $request->other_location_bt_perdiem[$key] ?? "";
                    $companyCode = $request->company_bt_perdiem[$key] ?? "";
                    $nominal = str_replace(
                        ".",
                        "",
                        $request->nominal_bt_perdiem[$key] ?? "0",
                    );
            ...
            if ($request->has("tanggal_bt_transport")) {
                foreach ($request->tanggal_bt_transport as $key => $tanggal) {
                    ...
                    $nominal = str_replace(
                        ".",
                        "",
                        $request->nominal_bt_transport[$key] ?? "0",
                    );
            ...
            if ($request->has("start_bt_penginapan")) {
                foreach ($request->start_bt_penginapan as $key => $startDate) {
                    ...
                    $nominal = str_replace(
                        ".",
                        "",
                        $request->nominal_bt_penginapan[$key] ?? "0",
                    );
            ...
            if ($request->has("tanggal_bt_lainnya")) {
                foreach ($request->tanggal_bt_lainnya as $key => $tanggal) {
                    ...
                    $nominal = str_replace(
                        ".",
                        "",
                        $request->nominal_bt_lainnya[$key] ?? "0",
                    );
            ...
            if ($statusValue !== "Draft") {
                ...
                $cek_director_id = Employee::select([
                    ...
                    "dsg2.director_flag",
                    DB::raw(
                        "SUBSTRING_INDEX(SUBSTRING_INDEX(dsg.department_level2, '(', -1), ')', 1) AS department_director",
                    ),
                    ...
                    ->leftJoin(
                        ...
                        "dsg.job_code",
                        "=",
                        "employees.designation_code",
                    )
                    ->leftJoin(
                        "designations as dsg2",
                        "dsg2.department_code",
                        "=",
                        DB::raw(
                            "SUBSTRING_INDEX(SUBSTRING_INDEX(dsg.department_level2, '(', -1), ')', 1)",
                        ),
                    )
                    ->leftJoin(
                        "employees as emp",
                ...
                foreach ($data_matrix_approvals as $data_matrix_approval) {
                    if ($data_matrix_approval->employee_id == "cek_L1") {
                        ...
                        $data_matrix_approval->employee_id == "cek_director"
                    ) {
            ...
            CATransaction::where("no_sppd", $oldNoSppd)
                ->where("type_ca", "dns")
                ->delete();
        }
        ...
        if ($request->ent === "Ya") {
            $businessTripStatus = $request->input("status");
            $ent = CATransaction::where("no_sppd", $oldNoSppd)
                ->where("type_ca", "entr")
                ->first();
            if (!$ent) {
            ...
            // dd($request->ca_decla);
            $ent->total_days = Carbon::parse($request->mulai)->diffInDays(
                Carbon::parse($request->kembali),
            );
            $ent->total_ca = (int) str_replace(
                ".",
                "",
                $request->total_ent_detail,
            );
            $ent->total_real = "0";
            $ent->total_cost = (int) str_replace(
                ".",
                "",
                $request->total_ent_detail,
            );
            ...
            $ent->approval_sett = $request->approval_sett
                ? $request->approval_sett
                : "";
            $ent->approval_extend = $request->approval_extend
                ? $request->approval_extend
                : "";
            ...
            if ($request->has("enter_type_e_detail")) {
                foreach ($request->enter_type_e_detail as $key => $type) {
                    ...
                    $nominal = str_replace(
                        ".",
                        "",
                        $request->nominal_e_detail[$key],
                    ); // Menghapus titik dari nominal sebelum menyimpannya
                        ...
                        $relation_e[] = [
                            ...
                            "relation_type" => array_filter(
                                [
                                    "Food" =>
                                        !empty(
                                            $request->food_e_relation[$key]
                                        ) &&
                                        $request->food_e_relation[$key] ===
                                            "food",
                                    "Transport" =>
                                        !empty(
                                            $request->transport_e_relation[$key]
                                        ) &&
                                        $request->transport_e_relation[$key] ===
                                            "transport",
                                    "Accommodation" =>
                                        !empty(
                                            $request->accommodation_e_relation[
                                                $key
                                            ]
                                        ) &&
                                        $request->accommodation_e_relation[
        ...
        if ($statusValue !== "Draft") {
            ...
            $managerName = Employee::where("employee_id", $managerL1)
                ->pluck("fullname")
                ->first();
            ...
            $employeeName = Employee::where("id", $n->user_id)
                ->pluck("fullname")
                ->first();
            $base64Image =
                "data:image/png;base64," . base64_encode($imageContent);
            $textNotification =
                "requesting a Business Travel and waiting for your approval with the following details :";
            ...
            if ($managerEmail) {
                ...
                $caDetails = [
                    "total_days_perdiem" => array_sum(
                        array_column(
                            $detail_ca_ntf["detail_perdiem"] ?? [],
                            "total_days",
                        ),
                    ),
                    "total_amount_perdiem" => array_sum(
                        array_column(
                            $detail_ca_ntf["detail_perdiem"] ?? [],
                            "nominal",
                        ),
                    ...
                    "total_days_transport" => count(
                        $detail_ca_ntf["detail_transport"] ?? [],
                    ),
                    "total_amount_transport" => array_sum(
                        array_column(
                            $detail_ca_ntf["detail_transport"] ?? [],
                            "nominal",
                        ),
                    ...
                    "total_days_accommodation" => array_sum(
                        array_column(
                            $detail_ca_ntf["detail_penginapan"] ?? [],
                            "total_days",
                        ),
                    ),
                    "total_amount_accommodation" => array_sum(
                        array_column(
                            $detail_ca_ntf["detail_penginapan"] ?? [],
                            "nominal",
                        ),
                    ...
                    "total_days_others" => count(
                        $detail_ca_ntf["detail_lainnya"] ?? [],
                    ),
                    "total_amount_others" => array_sum(
                        array_column(
                            $detail_ca_ntf["detail_lainnya"] ?? [],
                            "nominal",
                        ),
                    ...
                    "total_days_meals" => count(
                        $detail_ca_ntf["detail_meals"] ?? [],
                    ),
                    "total_amount_meals" => array_sum(
                        array_column(
                            $detail_ca_ntf["detail_meals"] ?? [],
                            "nominal",
                        ),
                ...
                $entDetails = [
                    "total_amount_ent" => array_sum(
                        array_column($detail_ent["detail_e"] ?? [], "nominal"),
                    ),
                ];
                ...
                try {
                    ...
                    Log::error(
                        "Email Create Business Travel tidak terkirim: " .
                            $e->getMessage() .
                            " | File: " .
                            $e->getFile() .
                            " | Line: " .
        ...
        return redirect("/businessTrip")->with(
            "success",
            "Business Travel updated successfully",
        );
    }
    ...
    public function deklarasi($id)
    {
        ...
        $job_level = Employee::where("id", $userId)
            ...
            ->first();
        $job_level_number = (int) preg_replace("/[^0-9]/", "", $job_level);
        $group_company = Employee::where("id", $employee_data->id)
            ->pluck("group_company")
            ->first();
        ...
        $nominalPerdiem = isset($caDetail["detail_perdiem"][0]["nominal"])
            ? $caDetail["detail_perdiem"][0]["nominal"]
            : "0";
        $nominalPerdiemDeclare = isset(
            $declareCa["detail_perdiem"][0]["nominal"],
        )
            ? $declareCa["detail_perdiem"][0]["nominal"]
            : "0";
    ...
    public function deklarasiCreate(Request $request, $id)
    {
        ...
        if ($request->has("removed_prove_declare")) {
            ...
            $existingFiles = $request->existing_prove_declare
                ? json_decode($request->existing_prove_declare, true)
                : [];
        ...
        if ($caRecords->isEmpty()) {
            if ($entrTab == false && $request->totalca > 0) {
                ...
                $ent->declare_estimate = Carbon::parse(
                    $request->kembali,
                )->addDays(3);
                $ent->total_days = Carbon::parse($request->mulai)->diffInDays(
                    Carbon::parse($request->kembali),
                ...
                $ent->total_ca = "0";
                $ent->total_real = (int) str_replace(
                    ".",
                    "",
                    $request->totalca,
                );
            ...
            if ($dnsTab == false && $request->totalca_ca_deklarasi > 0) {
                ...
                $ca->declare_estimate = Carbon::parse(
                    $request->kembali,
                )->addDays(3);
                $ca->total_days = Carbon::parse($request->mulai)->diffInDays(
                    Carbon::parse($request->kembali),
                ...
                $ca->total_ca = "0";
                $ca->total_real = (int) str_replace(
                    ".",
                    "",
                    $request->totalca_ca_deklarasi,
                );
                $ca->total_cost =
                ...
                if ($request->has("start_bt_perdiem")) {
                    foreach ($request->start_bt_perdiem as $key => $startDate) {
                        $endDate = $request->end_bt_perdiem[$key] ?? "";
                        $totalDays =
                            $request->total_days_bt_perdiem[$key] ?? "";
                        $location = $request->location_bt_perdiem[$key] ?? "";
                        $other_location =
                            $request->other_location_bt_perdiem[$key] ?? "";
                        $companyCode = $request->company_bt_perdiem[$key] ?? "";
                ...
                if ($request->has("tanggal_bt_transport")) {
                    foreach (
                        $request->tanggal_bt_transport
                        as $key => $tanggal
                    ) {
                        $keterangan =
                            $request->keterangan_bt_transport[$key] ?? "";
                        $companyCode =
                            $request->company_bt_transport[$key] ?? "";
                        $nominal = str_replace(
                            ".",
                            "",
                            $request->nominal_bt_transport[$key] ?? "0",
                ...
                if ($request->has("start_bt_penginapan")) {
                    foreach (
                        $request->start_bt_penginapan
                        as $key => $startDate
                    ) {
                        $endDate = $request->end_bt_penginapan[$key] ?? "";
                        $totalDays =
                            $request->total_days_bt_penginapan[$key] ?? "";
                        $hotelName =
                            $request->hotel_name_bt_penginapan[$key] ?? "";
                        $companyCode =
                            $request->company_bt_penginapan[$key] ?? "";
                        $nominal = str_replace(
                            ".",
                            "",
                            $request->nominal_bt_penginapan[$key] ?? "0",
                ...
                if ($request->has("tanggal_bt_lainnya")) {
                    foreach ($request->tanggal_bt_lainnya as $key => $tanggal) {
                        $keterangan =
                            $request->keterangan_bt_lainnya[$key] ?? "";
                        $type = $request->type_bt_lainnya[$key] ?? "";
                        $nominal = str_replace(
                            ".",
                            "",
                ...
                $ca->detail_ca =
                    '{"detail_perdiem":[],"detail_meals":[],"detail_transport":[],"detail_penginapan":[],"detail_lainnya":[]}';
        ...
        if ($caRecords) {
            foreach ($caRecords as $ca) {
                ...
                if ($ca->type_ca == "dns") {
                    ...
                    if ($request->has("start_bt_perdiem")) {
                        foreach (
                            ...
                            $totalDays =
                                $request->total_days_bt_perdiem[$key] ?? "";
                            $location =
                                $request->location_bt_perdiem[$key] ?? "";
                            $other_location =
                                $request->other_location_bt_perdiem[$key] ?? "";
                            $companyCode =
                                $request->company_bt_perdiem[$key] ?? "";
                            $nominal = str_replace(
                                ".",
                                "",
                                $request->nominal_bt_perdiem[$key] ?? "0",
                    ...
                    if ($request->has("start_bt_penginapan")) {
                        foreach (
                            ...
                            $totalPenginapan = str_replace(
                                ".",
                                "",
                                $request->total_bt_penginapan[$key] ?? "0",
                            );
                    ...
                    if ($request->has("tanggal_bt_lainnya")) {
                        foreach (
                            ...
                            $totalLainnya = str_replace(
                                ".",
                                "",
                                $request->total_bt_lainnya[$key] ?? "0",
                            );
                ...
                if ($entrTab == false && $request->totalca > 0) {
                    ...
                    $ca->total_days = Carbon::parse(
                        $request->mulai,
                    )->diffInDays(Carbon::parse($request->kembali));
                    $ca->total_ca = "0";
                    $ca->total_real = (int) str_replace(
                        ".",
                        "",
        ...
        if ($statusValue !== "Declaration Draft") {
            ...
            if ($managerEmail) {
                ...
                $detail_ca_req =
                    isset($dnsNtfRe) && isset($dnsNtfRe->detail_ca)
                        ? json_decode($dnsNtfRe->detail_ca, true)
                        : [];
                $detail_ent_req =
                    isset($entrNtfRe) && isset($entrNtfRe->detail_ca)
                        ? json_decode($entrNtfRe->detail_ca, true)
                        : [];
                ...
                $base64Image =
                    "data:image/png;base64," . base64_encode($imageContent);
                $textNotification =
                    "requesting a Declaration Business Travel and waiting for your approval with the following details :";
                ...
                $caDetails = [
                    ...
                    "total_days_transport" => count(
                        $detail_ca_req["detail_transport"] ?? [],
                    ),
                    "total_amount_transport" => array_sum(
                        array_column(
                            $detail_ca_req["detail_transport"] ?? [],
                    ...
                    "total_days_accommodation" => array_sum(
                        array_column(
                            $detail_ca_req["detail_penginapan"] ?? [],
                            "total_days",
                        ),
                    ),
                    "total_amount_accommodation" => array_sum(
                    ...
                    "total_days_meals" => array_sum(
                        array_column(
                            $detail_ca_req["detail_meals"] ?? [],
                            "total_days",
                        ),
                    ),
                    "total_amount_meals" => array_sum(
                ...
                $declare_ent_ntf = isset($declare_ent_ntf)
                    ? $declare_ent_ntf
                    : [];
                $caDeclare = [
                    "total_days_perdiem" => array_sum(
                        array_column(
                            $declare_ca_ntf["detail_perdiem"] ?? [],
                            "total_days",
                        ),
                    ),
                    "total_amount_perdiem" => array_sum(
                    ...
                    "total_days_transport" => count(
                        $declare_ca_ntf["detail_transport"] ?? [],
                    ),
                    "total_amount_transport" => array_sum(
                        array_column(
                            $declare_ca_ntf["detail_transport"] ?? [],
                    ...
                    "total_days_accommodation" => array_sum(
                        array_column(
                            $declare_ca_ntf["detail_penginapan"] ?? [],
                            "total_days",
                        ),
                    ),
                    "total_amount_accommodation" => array_sum(
                ...
                try {
                    ...
                    Log::error(
                        "Email Deklarasi Create Business Travel tidak terkirim: " .
                            $e->getMessage(),
                    );
                }
            }
    ...
    public function businessTripExtend(Request $req)
    {
        ...
        $model_ca_bt = CATransaction::where("no_sppd", $model_bt->no_sppd)
            ->where("type_ca", "dns")
            ->first();
        $model_ca_ent = CATransaction::where("no_sppd", $model_bt->no_sppd)
            ->where("type_ca", "entr")
            ->first();

        $dnsTab = $model_ca_bt ? true : false;
        $entrTab = $model_ca_ent ? true : false;
        ...
        if ($req->input("action_ca_submit")) {
            ...
            if ($dnsTab) {
                ...
                $cek_director_id = Employee::select([
                    ...
                    ->where(
                        "employees.designation_code",
                        "=",
                        $employee_data->designation_code,
                    )
                    ->where("dsg2.director_flag", "=", "T")
                    ->get();
                ...
                $data_matrix_approvals = MatrixApproval::where(
                    "modul",
                    "businesstripExtend",
                )
                    ->where(
                ...
                foreach ($data_matrix_approvals as $data_matrix_approval) {
                    ...
                    if ($employee_id != null) {
                        ...
                        $model_approval->ext_end_date = $req->input(
                            "ext_end_date",
                        );
                        $model_approval->total_days = $req->input("totaldays");
                        $model_approval->ext_total_days = $req->input(
                            "ext_totaldays",
                        );
            ...
            if ($entrTab) {
                $model_ca_ent->approval_extend = $req->input(
                    "action_ca_submit",
                );
            ...
            $approval_bt = new BTApproval();
            $approval_bt->id = (string) Str::uuid();
            $approval_bt->approval_status = "Request Extend";
            $approval_bt->bt_id = $model_bt->id;
            $approval_bt->layer = "0";
            $approval_bt->ext_end_date = $req->input("ext_end_date");
            $approval_bt->reason_extend = $req->input("ext_reason");
            $approval_bt->employee_id = $employee_data->employee_id;
            ...
            if ($statusValue == "Extend L1") {
                ...
                $imagePath = public_path("images/kop.jpg");
                $imageContent = file_get_contents($imagePath);
                $employeeName = Employee::where("id", $userId)
                    ->pluck("fullname")
                    ->first();
                $base64Image =
                    "data:image/png;base64," . base64_encode($imageContent);
                $textNotification =
                    "requesting Extend for his Business Travel and waiting for your approval with the following details :";
                $managerName = Employee::where("employee_id", $managerL1)
                    ->pluck("fullname")
                    ->first();

                if ($managerEmail) {
                    $ca = CATransaction::where("no_sppd", $model_bt->no_sppd)
                        ->orWhere("caonly", "!=", "Y")
                        ->get();
                    $dns = $ca->where("type_ca", "dns")->first();
                    $entr = $ca->where("type_ca", "entr")->first();
                    ...
                    if ($isCa == true) {
                        $detail_ca = $ca
                            ? json_decode($dns->detail_ca, true)
                            : [];
                        $caDetails = [
                            "total_days_perdiem" => array_sum(
                                array_column(
                                    $detail_ca["detail_perdiem"] ?? [],
                                    "total_days",
                                ),
                            ),
                            "total_amount_perdiem" => array_sum(
                    ...
                    if ($isEnt == true) {
                        $detail_ca = $ca
                            ? json_decode($entr->detail_ca, true)
                            : [];
                        $entDetails = [
                            "total_amount_ent" => array_sum(
                                array_column(
                    ...
                    $taksiDetails = Taksi::where(
                        "no_sppd",
                        $model_bt->no_sppd,
                    )->first();
                    // dd($taksiDetails);
                    $approvalLink = route("approve.business.trip.extend", [
                    ...
                    try {
                        ...
                        Log::error(
                            "Link Email Extend Bussines Trip tidak terkirim: " .
                                $e->getMessage(),
                        );
                    }
                }
            ...
            return redirect("/businessTrip")->with(
                "success",
                "Transaction asking for Extend, Please wait for Approval.",
            );
        }
    }
                ...
                foreach ($types as $type) {
                    ...
                    switch ($type) {
                        case "sppd":
                            $pdfName = "SPPD.pdf";
                            $viewPath =
                                "hcis.reimbursements.businessTrip.sppd_pdf";
                            $data = ["sppd" => $sppd];
                            break;
                        case "ca":
                            $ca = CATransaction::where(
                                "no_sppd",
                                $sppd->no_sppd,
                            )
                                ->where("approval_status", "!=", "Rejected")
                                ->first();
                            $allCa = CATransaction::where(
                                "no_sppd",
                                $sppd->no_sppd,
                            )
                                ->where("approval_status", "!=", "Rejected")
                                ->whereNotNull("date_required")
                                ->whereNotNull("status_id")
                                ->get();

                            if ($allCa->isEmpty()) {
                                Log::info(
                                    "Skipping CA download: No CA records found",
                                );
                                continue 2;
                            }
                            ...
                            if ($dnsCA) {
                                ...
                                $pdfFiles[] = [
                                    "name" => "CA.pdf",
                                    "viewPath" =>
                                        "hcis.reimbursements.businessTrip.ca_pdf",
                                    "data" => $data,
                                ];
                            }
                            $entrCA = $allCa
                            ...
                            if ($entrCA) {
                                ...
                                $pdfFiles[] = [
                                    ...
                                    "viewPath" =>
                                        "hcis.reimbursements.businessTrip.caEntr_pdf",
                                    "data" => $data,
                                ];
                                // }
                            }
                        ...
                        case "hotel":
                            $hotels = Hotel::where(
                                "no_sppd",
                                $sppd->no_sppd,
                            )->get(); // Fetch all hotels with the given sppd
                            if ($hotels->isEmpty()) {
                                continue 2; // Skip if no hotels found
                            }
                            $pdfName = "Hotel.pdf";
                            $viewPath =
                                "hcis.reimbursements.businessTrip.hotel_pdf";
                            $data = [
                        ...
                        case "mess":
                            ...
                            $pdfName = "Mess.pdf";
                            $viewPath =
                                "hcis.reimbursements.businessTrip.mess_pdf";
                            $data = [
                                "mess" => $messes->first(),
                                "messes" => $messes,
                            ];
                        ...
                        case "taksi":
                            ...
                            $pdfName = "Taxi.pdf";
                            $viewPath =
                                "hcis.reimbursements.businessTrip.taksi_pdf";
                            $data = ["taksi" => $taksi];
                            break;
                        case "deklarasi":
                            $ca = CATransaction::where(
                        ...
                        if ($type === "ca") {
                            ...
                            $canvas->page_script(
                                ...
                                if ($PAGE_COUNT > 2) {
                                    ...
                                    $size = 8;
                                    $color = array(0, 0, 0);
                                    $text = "Page " . $PAGE_NUM . " of " . $PAGE_COUNT . " Cash Advanced No. ' .
                                    $ca->no_ca .
                                    '";
    ...
    public function businessTripformAdd()
    {
        ...
        $no_sppds = CATransaction::where("user_id", $userId)
            ->where("approval_sett", "!=", "Done")
            ->get();
        $bt_sppd = BusinessTrip::where("status", "!=", "Verified")
            ->where("status", "!=", "Rejected")
            ->where("status", "!=", "Draft")
            ->orderBy("no_sppd", "desc")
            ->get();
    ...
    public function businessTripCreate(Request $request)
    {
        ...
        if ($tktDalam === "Ya") {
            ...
            foreach ($ticketData["dari_tkt"] as $key => $value) {
                if (!empty($value)) {
                    ...
                    $tiket->tgl_brkt_tkt =
                        $ticketData["tgl_brkt_tkt"][$key] ?? null;
                    $tiket->tgl_plg_tkt =
                        $ticketData["tgl_plg_tkt"][$key] ?? null;
                    $tiket->jam_brkt_tkt =
                        $ticketData["jam_brkt_tkt"][$key] ?? null;
                    $tiket->jam_plg_tkt =
                        $ticketData["jam_plg_tkt"][$key] ?? null;
        ...
        return redirect()
            ->route("businessTrip")
            ->with("success", "Request Successfully Added");
    }
    ...
    public function adminDivision(Request $request)
    {
        ...
        $sppdNos = $sppd->pluck("no_sppd");
        $btIds = $sppd->pluck("id");
        $departments = Designation::select("department_name")
            ->distinct()
            ->get();
        ...
        return view(
            "hcis.reimbursements.businessTrip.btAdminDivison",
            compact(
                "sppd",
                "parentLink",
                "link",
    ...
    public function exportExcelDivision(Request $request)
    {
        ...
        return Excel::download(
            new BusinessTripExport($businessTrips, $caData),
            "Data_Perjalanan_Dinas.xlsx",
        );
    }
    ...
    public function exportPdfDivision(Request $request)
    {
        ...
        // Generate PDF
        $pdf = PDF::loadView("hcis.reimbursements.businessTrip.division-pdf", [
    ...
    public function admin(Request $request)
    {
        ...
        if (!empty($permissionLocations)) {
            $query->whereHas("employee", function ($query) use (
                $permissionLocations,
            ) {
        ...
        return view(
            "hcis.reimbursements.businessTrip.btAdmin",
            compact(
                "sppd",
                "parentLink",
                "link",
    ...
    public function filterDateAdmin(Request $request)
    {
        ...
        $extendData = BTApproval::whereIn("bt_id", $sppd->pluck("id"))
            ->where("approval_status", "Request Extend")
            ->orderBy("created_at", "asc")
            ->get(["bt_id", "ext_end_date", "reason_extend"]);

        $extendTime = $extendData->keyBy("bt_id")->map(function ($item) {
            return [
    ...
    public function deklarasiStatusAdmin(Request $request, $id)
    {
        ...
        $accNum = Company::where("contribution_level_code", $n->bb_perusahaan)
        ...
        if ($request->accept_status !== "Request Revision") {
            if ($caRecords) {
                foreach ($caRecords as $ca) {
                    // Assign or update values to $ca model
                    if ($ca->type_ca == "dns") {
                        ...
                        if ($request->has("start_bt_penginapan")) {
                            foreach (
                                ...
                                $nominal = str_replace(
                                    ".",
                                    "",
                                    $request->nominal_bt_penginapan[$key] ??
                                        "0",
                                );
                        ...
                        if (
                            $ca->total_cost <= 0 &&
                            $request->input("accept_status") === "Return/Refund"
                        ) {
                            return redirect()
                                ->back()
                                ->with(
                                    "error",
                                    "Cannot set status to Return/Refund when the Total Cost is negative.",
                                );
                        } elseif (
                            $ca->total_cost > 0 &&
                            $request->input("accept_status") === "Return/Refund"
                        ) {
                            $employeeEmail = Employee::where("id", $n->user_id)
                                ->pluck("email")
                                ->first();
                        ...
                        if ($request->has("enter_type_e_detail")) {
                            foreach (
                                ...
                                $fee_detail =
                                    $request->enter_fee_e_detail[$key];
                                $nominal = str_replace(
                                    ".",
                                    "",
                                    $request->nominal_e_detail[$key],
                        ...
                        if ($request->has("rname_e_relation")) {
                            foreach (
                                $request->rname_e_relation
                                as $key => $name
                            ) {
                                $position =
                                    $request->rposition_e_relation[$key];
                                $company = $request->rcompany_e_relation[$key];
                                $purpose = $request->rpurpose_e_relation[$key];
                ...
                if ($employeeEmail) {
                    ...
                    $entDetails = [
                        "total_amount_detailent" => array_sum(
                            array_column(
                                $detail_ent_req["detail_e"] ?? [],
                                "nominal",
                            ),
                    ...
                    $declare_ca_ntf =
                        isset($dnsNtfRe) && isset($dnsNtfRe->detail_ca)
                            ? json_decode($dnsNtfRe->declare_ca, true)
                            : [];
                    $declare_ent_ntf =
                        isset($entrNtfRe) && isset($entrNtfRe->detail_ca)
                            ? json_decode($entrNtfRe->declare_ca, true)
                            : [];
                    $caDeclare = [
                    ...
                    $selisihEnt =
                        array_sum($entDetails) - array_sum($entDeclare);
                    ...
                    try {
                        ...
                        Log::error(
                            "Email Deklarasi Status Admin Business Travel tidak terkirim: " .
                                $e->getMessage(),
                        );
                    }
                }
            ...
            // Step 1: Update BusinessTrip
            $n->status = "Approved";
            $n->revision_at = Carbon::now();

            if (!empty($ca_note)) {
                $n->revision_note = $ca_note;
            }
            $n->save();

            // Step 2: Update related approvals using Eloquent
            $n->approvals()
            ...
            // Step 3 : update data di catransaction
            foreach ($caRecords as $ca) {
                $ca->sett_id = "";
                $ca->approval_sett = "";
                $ca->ca_status = "-";
            ...
            // Step 4 : update CA Approval
            foreach ($caRecords as $ca) {
                $ca->settApprovals()->update(["deleted_at" => Carbon::now()]);
            }
        }

        // return redirect('/businessTrip/admin')->with('success', 'Status updated successfully');
        return redirect(
            session("previous_url", url("businessTrip/admin")),
    ...
    public function exportExcel(Request $request)
    {
        ...
        $queryCA->where("approval_status", "<>", "draft");
        ...
        // Extract the no_sppd values from the filtered BusinessTrip records
        $noSppds = $businessTrips->pluck("no_sppd")->unique();
        $idBt = $businessTrips->pluck("id")->unique();

        // Fetch CA data where no_sppd matches the filtered BusinessTrip records
        $caData = $queryCA->whereIn("no_sppd", $noSppds)->get();
        $btApprovalData = $queryBtApproval->whereIn("bt_id", $idBt)->get();

        // Pass the filtered data to the export class
        return Excel::download(
            new BusinessTripExport($businessTrips, $caData, $btApprovalData),
            "Data_Perjalanan_Dinas.xlsx",
        );
    }
    ...
    public function approval(Request $request)
    {
        ...
        $bt_all = BusinessTrip::where(function ($query) use ($user) {
            $query
                ->where(function ($q) use ($user) {
                    $q->where("manager_l1_id", $user->employee_id)->whereIn(
                        "status",
                        ["Pending L1", "Declaration L1", "Extend L1"],
                    );
                })
                ->orWhere(function ($q) use ($user) {
                    $q->where("manager_l2_id", $user->employee_id)->whereIn(
                        "status",
                        ["Pending L2", "Declaration L2", "Extend L2"],
                    );
                });
        })
            ->orderBy("created_at", "desc")
        ...
        $bt_request = BusinessTrip::where(function ($query) use ($user) {
            $query
                ->where(function ($q) use ($user) {
                    $q->where("manager_l1_id", $user->employee_id)->where(
                        "status",
                        "Pending L1",
                    );
                })
                ->orWhere(function ($q) use ($user) {
                    $q->where("manager_l2_id", $user->employee_id)->where(
                        "status",
                        "Pending L2",
                    );
                });
        ...
        $bt_extend = BusinessTrip::where(function ($query) use ($user) {
            $query
                ->where(function ($q) use ($user) {
                    $q->where("manager_l1_id", $user->employee_id)->where(
                        "status",
        ...
        $extendCount = $bt_extend->count();
        $totalBTCount = $requestCount + $declarationCount + $extendCount;
        $totalPendingCount = CATransaction::where(function ($query) use (
            $employeeId,
        ) {
            $query
                ->where("status_id", $employeeId)
                ->where("approval_status", "Pending")
                ->orWhere("sett_id", $employeeId)
                ->where("approval_sett", "Pending")
                ->orWhere("extend_id", $employeeId)
        ...
        $transactions_tkt = Tiket::whereIn("no_tkt", $ticketNumbers)
            ...
            ->orderBy("created_at", "desc")
            ->get();
        $totalTKTCount = $transactions_tkt
            ->filter(function ($ticket) use ($employee) {
                $ticketOwnerEmployee = Employee::where(
                    "id",
                    $ticket->user_id,
                )->first();
                return ($ticket->approval_status == "Pending L1" &&
                    $ticket->manager_l1_id == $employee->employee_id) ||
                    ($ticket->approval_status == "Pending L2" &&
                        $ticket->manager_l2_id == $employee->employee_id);
            })
            ->count();
        ...
        $hotels = $transactions_htl->filter(function ($hotel) use ($employee) {
            ...
            if (
                $hotel->approval_status == "Pending L1" &&
                $hotel->manager_l1_id == $employee->employee_id
            ) {
                return true;
            } elseif (
                $hotel->approval_status == "Pending L2" &&
                $hotel->manager_l2_id == $employee->employee_id
            ) {
                return true;
            }
            return false;
        ...
        $hasApprovalRights = DB::table("master_bisnisunits")
            ->whereRaw("FIND_IN_SET(?, approval_medical)", [
        ...
        if ($hasApprovalRights) {
            $medicalGroup = HealthCoverage::from(
                "mdc_transactions as mdc_transactions",
            )
                ->join(
                    "employees as e",
                ...
                ->select(
                    ...
                    DB::raw(
                        'SUM(CASE WHEN mdc_transactions.medical_type = "Maternity" THEN mdc_transactions.balance_verif ELSE 0 END) as maternity_balance_verif',
                    ),
                    DB::raw(
                        'SUM(CASE WHEN mdc_transactions.medical_type = "Inpatient" THEN mdc_transactions.balance_verif ELSE 0 END) as inpatient_balance_verif',
                    ),
                    DB::raw(
                        'SUM(CASE WHEN mdc_transactions.medical_type = "Outpatient" THEN mdc_transactions.balance_verif ELSE 0 END) as outpatient_balance_verif',
                    ),
                    DB::raw(
            ...
            $medical = $medicalGroup->map(function ($item) {
                ...
                $item->total_per_no_medic =
                    $item->maternity_balance_verif +
                    $item->inpatient_balance_verif +
                    $item->outpatient_balance_verif +
                    $item->glasses_balance_verif;
        ...
        $caTransactions = CATransaction::whereIn("no_sppd", $sppdNos)
            ->whereNull("deleted_at")
            ->get()
            ->groupBy("no_sppd");

        $extendData = BTApproval::whereIn("bt_id", $bt_extend->pluck("id"))
            ->where("approval_status", "Request Extend")
            ->orderBy("created_at", "asc")
            ->get(["bt_id", "ext_end_date", "reason_extend"]);
        ...
        $managerL1Names = Employee::whereIn(
            "employee_id",
            $bt_all->pluck("manager_l1_id"),
        )->pluck("fullname", "employee_id");
        $managerL2Names = Employee::whereIn(
            "employee_id",
            $bt_all->pluck("manager_l2_id"),
        )->pluck("fullname", "employee_id");
        ...
        return view(
            "hcis.reimbursements.businessTrip.btApproval",
            compact(
                "bt_all",
                "bt_request",
                "bt_declaration",
    ...
    public function approvalDetail($id)
    {
        ...
        $bt_sppd = BusinessTrip::where("status", "!=", "Done")
            ...
            ->orderBy("no_sppd", "desc")
            ->get();
        $job_level = Employee::where("id", $employee_data)
            ->pluck("job_level")
            ->first();
        $job_level_number = (int) preg_replace("/[^0-9]/", "", $job_level);
    ...
    public function updateStatus($id, Request $request)
    {
        ...
        if ($action == "Request Revision") {
            ...
            if ($businessTrip->ca == "Ya") {
                $caTransaction = CATransaction::where(
                    "no_sppd",
                    $businessTrip->no_sppd,
                )->get();
            ...
            $layer = 1;
            $managerL2 = Employee::where(
                "employee_id",
                $businessTrip->manager_l2_id,
            )
                ->pluck("email")
                ->first();
            ...
            $managerName = Employee::where(
                ...
                ->pluck("fullname")
                ->first();
            $group_company = Employee::where("id", $businessTrip->user_id)
                ->pluck("group_company")
                ->first();

            $imagePath = public_path("images/kop.jpg");
            $imageContent = file_get_contents($imagePath);
            $employeeName = Employee::where("id", $businessTrip->user_id)
                ->pluck("fullname")
                ->first();
            $base64Image =
                "data:image/png;base64," . base64_encode($imageContent);
            ...
            // dd($managerL2);
            if ($managerL2) {
                $ca = CATransaction::where("no_sppd", $businessTrip->no_sppd)
                    ->orWhere("caonly", "!=", "Y")
                    ->first();
                ...
                try {
                    ...
                    Log::error(
                        "Email Update Status Business Travel tidak terkirim: " .
                            $e->getMessage(),
                    );
                }
            }
            ...
            if ($businessTrip->tiket == "Ya") {
                ...
                foreach ($tikets as $tiket) {
                    if ($tiket->tkt_only != "Y") {
                        ...
                        $approval_tkt->role_name = Auth::user()->role_name; // Assuming role_name is in the user data
                        $approval_tkt->layer =
                            $tiket->approval_status == "Pending L2" ? 1 : 2; // Determine layer based on status
                        $approval_tkt->approval_status = $statusValue;
                        $approval_tkt->approved_at = now();
                        $approval_tkt->save();
                    }
        ...
        return redirect("/businessTrip/approval")->with(
            "success",
    ...
    public function adminApprove(Request $request, $id)
    {
        ...
        if (
            $businessTrip->status == "Pending L1" ||
            $businessTrip->status == "Pending L2"
        ) {
            if ($businessTrip->manager_l2_id == "-") {
                ...
                if ($businessTrip->ca == "Ya") {
                    ...
                    foreach ($caTransaction as $caTransactions) {
                        if (
                            ($caTransactions &&
                                $caTransactions->caonly != "Y") ||
                            $caTransactions->caonly == null
                        ) {
                            // Update CA approval status for L2
                ...
                $managerName = Employee::where(
                    ...
                    ->pluck("fullname")
                    ->first();
                $group_company = Employee::where("id", $businessTrip->id)
                    ->pluck("group_company")
                    ->first();
                ...
                if ($managerL2) {
                    $detail_ca = $isCa
                        ? json_decode($isCa->detail_ca, true)
                        : [];
                    $detail_ent = $isEnt
                        ? json_decode($isEnt->detail_ca, true)
                        : [];
                    $caDetails = [
                        "total_days_perdiem" => array_sum(
                            array_column(
        ...
        if (
            $businessTrip->status == "Declaration L1" ||
            $businessTrip->status == "Declaration L2"
        ) {
            if ($businessTrip->manager_l2_id == "-") {
                ...
                foreach ($caTransaction as $caTransactions) {
                    if (
                        ($caTransactions && $caTransactions->caonly != "Y") ||
                        $caTransactions->caonly == null
                    ) {
                        // Update CA approval status for L1
                        ...
                        // Find the next approver (Layer 3) explicitly
                        $nextApproval = ca_sett_approval::where(
                            "ca_id",
                            $caTransactions->id,
                        )
                            ->where("layer", $layer + 1) // This will ensure it gets the immediate next layer (3)
                ...
                $imagePath = public_path("images/kop.jpg");
                $imageContent = file_get_contents($imagePath);
                $employeeName = Employee::where("employee_id", $employeeId)
                    ->pluck("fullname")
                    ->first();
                $base64Image =
                    "data:image/png;base64," . base64_encode($imageContent);
                ...
                $declare_ca_ntf =
                    isset($dnsNtfRe) && isset($dnsNtfRe->declare_ca)
                        ? json_decode($dnsNtfRe->declare_ca, true)
                        : [];
                $declare_ent_ntf =
                    isset($entrNtfRe) && isset($entrNtfRe->detail_ca)
                ...
                if ($managerL2) {
                    ...
                    try {
                        ...
                        Log::error(
                            "Email Update Status Deklarasi Business Travel tidak terkirim: " .
                                $e->getMessage(),
                        );
                    }
                }
        ...
        if (
            $businessTrip->status == "Extend L1" ||
            $businessTrip->status == "Extend L2"
        ) {
            if ($businessTrip->manager_l2_id == "-") {
                ...
                foreach ($caTransaction as $caTransactions) {
                    if (
                        ...
                        if ($caApproval) {
                            // Only update if the record exists
                            $caApproval->approval_status = "Approved";
                            $caApproval->approved_at = now();
                            $caApproval->by_admin = "T";
                            $caApproval->admin_id = $employeeId;
                            $caApproval->save();
                        }

                        // Find the next approver (Layer 3) explicitly
                        $nextApproval = ca_extend::where(
                            "ca_id",
                            $caTransactions->id,
                        )
                            ->where("layer", $layer + 1) // This will ensure it gets the immediate next layer (3)
                        ...
                        if ($nextApproval) {
                            ...
                            $updateCa->extend_id = $nextApproval->employee_id;
                            $updateCa->save();
                        } else {
                            // No next layer, so mark as Approved
                            $caTransactions->update([
                                "approval_extend" => "Approved",
                            ]);
                        }
                    }
                }
                ...
                $businessTrip->update([
                    "status" => "Approved",
                    "kembali" => $extendInfo->ext_end_date,
                ]);
                $approval->bt_id = $businessTrip->id;
                $approval->layer = $layer;
                $approval->approval_status = $statusValue;
                ...
                // Save the approval record
                $approval->save();
            } elseif ($businessTrip->status == "Extend L1") {
                $statusValue = "Extend L2";
                $layer = 1;
                // Handle CA approval for L1

                $extendInfo = BTApproval::where("bt_id", $businessTrip->id)
                    ->where("approval_status", "Request Extend")
                    ->orderBy("created_at", "desc") // Mengurutkan dari terbaru
                    ->first();
                ...
                if ($managerL2) {
                    ...
                    $base64Image =
                        "data:image/png;base64," . base64_encode($imageContent);
                    $textNotification =
                        "requesting for Extend Business Travel and waiting for your Approval with the following details :";
                    $group_company = Employee::where(
                        "id",
                        $businessTrip->user_id,
                    )
                        ->pluck("group_company")
                ...
                // Save the approval record
                $approval->save();
            } elseif ($businessTrip->status == "Extend L2") {
                $statusValue = "Extend Approved";
                $layer = 2;
    ...
    public function adminRevisi(Request $request, $id)
    {
        ...
        return redirect("/businessTrip/admin")->with(
            "success",
    ...
    public function adminReject(Request $request, $id)
    {
        ...
        if ($request->input("action_ca_reject")) {
            $statusValue = "Extend Rejected";
            if ($businessTrip->status == "Extend L1") {
                $layer = 1;
            } elseif ($businessTrip->status == "Extend L2") {
                $layer = 2;
            } else {
            ...
            foreach ($caTransaction as $caTransactions) {
                if (
                    ...
                    if ($caApproval) {
                        ...
                        ca_extend::where("ca_id", $caTransactions->id)
            ...
            $approval_bt = new BTApproval();
            $approval_bt->id = (string) Str::uuid();
            $approval_bt->approval_status = $statusValue;
            $approval_bt->bt_id = $businessTrip->id;
            $approval_bt->layer = $layer;
            $approval_bt->ext_end_date = $extendInfo->ext_end_date;
            $approval_bt->reason_extend = $extendInfo->reason_extend;
            $approval_bt->employee_id = $employeeId;
            $approval_bt->employee_id = $employeeId;
            $approval_bt->by_admin = "T";

            $approval_bt->save();

            return redirect("/businessTrip/admin")->with(
                "success",
                "Extend Rejected, Rejection will be send to the employee.",
            );
        }
    ...
    public function updateStatusDeklarasi($id, Request $request)
    {
        ...
        if ($action == "Declaration Revision") {
            ...
            $caTrans = CATransaction::where("no_sppd", $businessTrip->no_sppd)
                ->where(function ($query) {
                    $query->where("caonly", "!=", "Y")->orWhereNull("caonly");
                })
                ->get();
            $dnsNtfRe = $caTrans->where("type_ca", "dns")->first();
            $entrNtfRe = $caTrans->where("type_ca", "entr")->first();
            ...
            $employeeName = Employee::where("employee_id", $employeeId)
                ->pluck("fullname")
                ->first();
            $group_company = Employee::where("employee_id", $employeeId)
                ->pluck("group_company")
                ->first();
            $base64Image =
                "data:image/png;base64," . base64_encode($imageContent);
            ...
            $entDetails = [
                "total_amount_ent" => array_sum(
                    array_column($detail_ent_req["detail_e"] ?? [], "nominal"),
                ),
            ];
            // dd($caDetails,   $detail_ca );
            ...
            $entDeclare = [
                "total_amount_ent" => array_sum(
                    array_column($declare_ent_ntf["detail_e"] ?? [], "nominal"),
                ),
            ];
            // dd($managerL2);
            if ($managerL2) {
    ...
    public function updateStatusExtended(Request $req)
    {
        $id = $req->input("no_id"); // Get the ID from the no_id input
        $userId = Auth::id();
        $employeeId = auth()->user()->employee_id;
        ...
        if ($req->input("action_ca_reject")) {
            if (
                !in_array($employeeId, [
                    $model_bt->manager_l1_id,
                    $model_bt->manager_l2_id,
                ])
            ) {
                return redirect()
                    ->back()
            ...
            $layer = $employeeId == $model_bt->manager_l1_id ? 1 : 2;
            $statusValue = "Extend Rejected";

            foreach (
                [$dnsTab => $model_ca_bt, $entrTab => $model_ca_ent]
                as $tab => $model_ca
            ) {
                if ($tab) {
                    ca_extend::where("ca_id", $model_ca->id)
                        ->whereNotIn("approval_status", ["Approved"])
                        ->whereNull("approved_at")
                        ->update([
                            "approval_status" => "Rejected",
                    ...
                    CATransaction::where("id", $model_ca->id)->update([
                        "approval_extend" => "Rejected",
                    ]);
                }
            }

            $model_bt->update(["status" => "Approved"]);
        ...
        if ($req->input("action_ca_approve")) {
            if ($employeeId == $model_bt->manager_l1_id) {
                $layer = 1;
                $statusValue = "Extend L2";
            } elseif ($employeeId == $model_bt->manager_l2_id) {
                $layer = 2;
                $statusValue = "Approved";
                $model_bt->update([
                    "status" => $statusValue,
                    "kembali" => $req->input("ext_end_date"),
                ]);
            } else {
                return redirect()
                    ->back()
            ...
            if ($dnsTab) {
                ...
                $approvals = ca_extend::where("ca_id", $model_ca_bt->id)
                    ->orderBy("layer", "asc") // Mengurutkan berdasarkan layer
                    ->get();

                $model = ca_extend::where("ca_id", $model_ca_bt->id)
                ...
                if (!$nextApproval) {
                    ...
                    if ($caTransaction) {
                        ...
                        $caTransaction->start_date = $req->input(
                            "ext_start_date",
                        );
                        $caTransaction->end_date = $req->input("ext_end_date");
                        $caTransaction->total_days = $req->input(
                            "ext_totaldays",
                        );
                        $caTransaction->reason_extend = $req->input(
                            "ext_reason",
                        );
                        // dd($caTransaction);
                        $caTransaction->save();
            ...
            if ($entrTab) {
                // Ambil semua approval yang terkait dengan ca_id
                $approvals = ca_extend::where("ca_id", $model_ca_ent->id)
                    ->orderBy("layer", "asc") // Mengurutkan berdasarkan layer
                    ->get();

                $model = ca_extend::where("ca_id", $model_ca_ent->id)
                    ->where("employee_id", $employeeId)
                    ->where("approval_status", "<>", "Rejected")
                    ->firstOrFail();
            ...
            // dd($statusValue);
            $model_bt->update(["status" => $statusValue]);
    ...
    public function filterDateApproval(Request $request)
    {
        ...
        $query = BusinessTrip::query()->where(function ($query) use ($user) {
            $query
                ->where(function ($q) use ($user) {
                    $q->where("manager_l1_id", $user->employee_id)->whereIn(
                        "status",
                        ["Pending L1", "Declaration L1"],
                    );
                })
                ->orWhere(function ($q) use ($user) {
                    $q->where("manager_l2_id", $user->employee_id)->whereIn(
                        "status",
                        ["Pending L2", "Declaration L2"],
                    );
                });
        });
        ...
        if ($filter === "request") {
            $query->where(function ($q) use ($user) {
                $q->where(function ($subQ) use ($user) {
                    ...
                    $subQ
                        ->where("manager_l2_id", $user->employee_id)
    ...
    private function generateNoSppd()
    {
        ...
        $lastTransaction = BusinessTrip::whereYear("created_at", $currentYear)
            ...
            ->orderByRaw(
                "CAST(SUBSTRING_INDEX(no_sppd, '/', 1) AS UNSIGNED) DESC",
            )
            ->withTrashed()
            ->first();
        ...
        if (
            $lastTransaction &&
            preg_match(
                "/(\d{1,5})\/SPPD-HC\/([IVX]+)\/\d{4}/",
                $lastTransaction->no_sppd,
                $matches,
            )
        ) {
    ...
    private function generateNoCa()
    {
        ...
        $lastNumber =
            $lastTransaction &&
            preg_match(
                "/CA" . $currentYearShort . "(\d{6})/",
                $lastTransaction->no_ca,
                $matches,
            )
                ? intval($matches[1])
                : 0;
    ...
    private function findDepartmentHead($employee)
    {
        ...
        if ($priority) {
            $manager = ApprovalPriority::where(
                "employee_id",
    ...
    private function UpdateDetailEmployees($employeeId)
    {
        ...
        $data = [
            "api_key" =>
                "46313f36ab8a8bc5aad64ff1c80c769a07716d9af8f07850f6ad2465a0c991b4d42e84414b5775ba151e7f2833223bfb1e0ecf49b89c7d6d0f6a6d39231666f8",
            "datasetKey" =>
                "11f8ded39d3f22e7c71900d90605c7bf8ef211ac94956b07cc2f8c340d61a4528342feb5afc4b9a9db0b980427007db0dd61db52ae857699d80d9c79a28078cc",
        ...
        try {
            ...
            if (empty($employees)) {
                Log::info("No employees data returned from API", [
                    "start" => $start,
                ]);
                return;
            }
            ...
            foreach ($employees as $employee) {
                Employee::where(
                    ...
                    "contribution_level_code" =>
                        $employee["contribution_level_code"],
                    ...
                    "bpjs_tk" =>
                        $employee[
                            "badan_penyelenggara_jaminan_sosial_(bpjs)_tenaga_kerja_(bpjstk)"
                        ],
                    "bpjs_ks" =>
                        $employee[
                            "badan_penyelenggara_jaminan_sosial_(bpjs)_kesehatan_(bpjskes)"
                        ],
            ...
            Log::error("Exception occurred during API fetch", [
                "error" => $e->getMessage(),
            ]);
        }
    }
