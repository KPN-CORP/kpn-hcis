...
class ReimburseController extends Controller
{
    ...
    public function cashadvanced()
    {
        ...
        $link = "Cash Advanced";
        $today = Carbon::today();
        $ca_transactions = CATransaction::with([
            "employee",
            "statusReqEmployee",
        ])
        ...
        foreach ($ca_transactions as $transaction) {
            $transaction->settName = $transaction->statusReqEmployee
                ? $transaction->statusReqEmployee->fullname
                : "";
        }
        // dd($disableCACount);
    ...
    public function requestCashadvanced()
    {
        ...
        $fullnames = Employee::whereIn(
            "employee_id",
            $ca_transactions->pluck("status_id"),
    ...
    public function cashadvancedAdmin(Request $request)
    {
        ...
        $isCreatedDateFilterApplied =
            $request->has(["from_date", "until_date"]) &&
            $request->get("from_date") != "" &&
            $request->get("until_date") != "";
        $isStatusFilterApplied =
            $request->has("stat") &&
            $request->get("stat") != "" &&
            $request->get("stat") != "-";
        $isStatusCaFilterApplied =
            $request->has("ca_status") &&
            $request->get("ca_status") != "" &&
            $request->get("ca_status") != "-";

        // Tentukan apakah filter telah diaplikasikan
        $isFilterApplied =
            $isDateRangeFilterApplied || $isCreatedDateFilterApplied;
        ...
        if ($isFilterApplied) {
            ...
            $query = CATransaction::with([
                "employee",
                "statusReqEmployee",
                "statusSettEmployee",
                "statusExtendEmployee",
            ...
            // Terapkan filter tanggal mulai-akhir
            ...
            // Terapkan filter tanggal pembuatan
            if ($isCreatedDateFilterApplied) {
                $fromDate = $request->input("from_date");
                $untilDate = Carbon::parse(
            ...
            if ($isStatusCaFilterApplied) {
                $statusca = $request->input("ca_status");
                if ($statusca === "nonca") {
                    $query->where("total_ca", 0);
                } elseif ($statusca === "ca") {
                    $query->where("total_ca", ">", 0);
                }
            }
            ...
            foreach ($ca_transactions as $transaction) {
                $transaction->ReqName = $transaction->statusReqEmployee
                    ? $transaction->statusReqEmployee->fullname
                    : "";
                $transaction->settName = $transaction->statusSettEmployee
                    ? $transaction->statusSettEmployee->fullname
                    : "";
                $transaction->extName = $transaction->statusExtendEmployee
                    ? $transaction->statusExtendEmployee->fullname
                    : "";
                $transaction->formatted_start_date = Carbon::parse(
                    $transaction->start_date,
                )->format("d-m-Y");
                $transaction->formatted_end_date = Carbon::parse(
            ...
            foreach ($ca_approvals as $approval) {
                $approval->ReqName = $approval->statusReqEmployee
                    ? $approval->statusReqEmployee->fullname
                    : "";
            }

            $ca_sett = ca_sett_approval::where(
                "approval_status",
                "<>",
                "Rejected",
            )
            ...
            foreach ($ca_sett as $approval_sett) {
                $approval_sett->ReqName = $approval_sett->statusReqEmployee
                    ? $approval_sett->statusReqEmployee->fullname
                    : "";
            }
            ...
            foreach ($ca_extend as $approval_ext) {
                $approval_ext->ReqName = $approval_ext->statusReqEmployee
                    ? $approval_ext->statusReqEmployee->fullname
                    : "";
            }
        }
        ...
        return view("hcis.reimbursements.cashadv.adminCashadv", [
            ...
            "ca_sett" => $ca_sett,
            "ca_extend" => $ca_extend,
            "isFilterApplied" => $isFilterApplied, // Tambahkan variabel ini ke view
        ]);
    }
    ...
    public function deklarasiCashadvanced()
    {
        ...
        $reason = ca_sett_approval::whereIn(
            "ca_id",
            $ca_transactions->pluck("id"),
    ...
    public function filterCaTransactions(Request $request)
    {
        ...
        return view(
            "hcis.reimbursements.cashadv.CaTransactionsTable",
            compact("ca_transactions"),
        )->render();
    }
    ...
    public function cashadvancedSubmit(Request $req)
    {
                    ...
                    if (
                        ...
                        $relation_e[] = [
                            ...
                            "relation_type" => array_filter(
                                [
                                    "Food" =>
                                        !empty($req->food_e_relation[$key]) &&
                                        $req->food_e_relation[$key] === "food",
                                    "Transport" =>
                                        !empty(
                                            $req->transport_e_relation[$key]
                                        ) &&
                                        $req->transport_e_relation[$key] ===
                                            "transport",
                                    "Accommodation" =>
                                        !empty(
                                            $req->accommodation_e_relation[$key]
                                        ) &&
                                        $req->accommodation_e_relation[$key] ===
                                            "accommodation",
                                    "Gift" =>
                                        !empty($req->gift_e_relation[$key]) &&
                                        $req->gift_e_relation[$key] === "gift",
                                    "Fund" =>
        ...
        if ($req->input("action_ca_draft")) {
            ...
            return redirect()
                ->route("cashadvanced")
                ->with("success", "Transaction successfully Added in Draft.");
        }
        ...
        if ($req->input("action_ca_submit")) {
            ...
            $data_matrix_approvals = MatrixApproval::where(function (
                $query,
            ) use ($req) {
                if ($req->ca_type === "dns") {
            ...
            if ($CANotificationLayer) {
                ...
                try {
                    ...
                    Log::error(
                        "Email Submit Cash Advanced tidak terkirim: " .
                            $e->getMessage(),
                    );
                }
            }
        ...
        if ($req->ca_type == "dns") {
            ...
            if ($req->has("start_bt_perdiem")) {
                foreach ($req->start_bt_perdiem as $key => $startDate) {
                    ...
                    $location = $req->location_bt_perdiem[$key];
                    $other_location =
                        $req->other_location_bt_perdiem[$key] ?? "";
                    $companyCode = $req->company_bt_perdiem[$key];
                    $nominal = str_replace(
                        ".",
                        "",
        ...
        if ($req->input("action_ca_submit")) {
            ...
            if ($CANotificationLayer) {
                ...
                try {
                    ...
                    Log::error(
                        "Email Edit Cash Advance tidak terkirim: " .
                            $e->getMessage(),
                    );
                }
            }
    ...
    public function cashadvancedExtend(Request $req)
    {
        ...
        if ($req->input("action_ca_submit")) {
            ...
            $nextApproval = ca_extend::where("ca_id", $id)
                ->where("employee_id", $managerL1)
                ->firstOrFail();
            ...
            if ($CANotificationLayer) {
                ...
                try {
                    ...
                    Log::error(
                        "Email Submit Extend Cash Advance tidak terkirim: " .
                            $e->getMessage(),
                    );
                }
            }
    ...
    function cashadvancedDeclare(Request $req, $key)
    {
        ...
        if ($req->has("removed_prove_declare")) {
            ...
            $existingFiles = $req->existing_prove_declare
                ? json_decode($req->existing_prove_declare, true)
                : [];
        ...
        if ($req->input("action_ca_submit")) {
            ...
            if ($CANotificationLayer) {
                ...
                try {
                    ...
                    Log::error(
                        "Email Submit Deklarasi Cash Advanced tidak terkirim: " .
                            $e->getMessage(),
                    );
                }
            }
    ...
    public function hotel(Request $request)
    {
        ...
        $query = Hotel::where("user_id", $userId->id)->orderBy(
            "created_at",
            "desc",
        );
    ...
    public function hotelSubmit(Request $req)
    {
        ...
        foreach ($hotelData["nama_htl"] as $key => $value) {
            ...
            if (
                !empty($hotelData["nama_htl"][$key]) &&
                !empty($hotelData["lokasi_htl"][$key]) &&
                !empty($hotelData["tgl_masuk_htl"][$key])
            ) {
                $model = new Hotel();
                ...
                $model->tgl_masuk_htl =
                    $hotelData["tgl_masuk_htl"][$key] ?? null;
        ...
        if ($statusValue !== "Draft") {
            $managerId = Employee::where("id", $userId)
                ->pluck("manager_l1_id")
                ->first();
            ...
            $base64Image =
                "data:image/png;base64," . base64_encode($imageContent);
            $textNotification =
                "requesting a Hotel and waiting for your Approval with the following details :";
            ...
            if ($managerEmail) {
                ...
                try {
                    ...
                    Log::error(
                        "Email Submit Hotel Draft tidak terkirim: " .
                            $e->getMessage(),
                    );
                }
            }
        ...
        return redirect("/hotel")->with(
            "success",
            "Hotel request input successfully",
        );
    }
    ...
    public function approveHotelFromLink($id, $manager_id, $status)
    {
        ...
        if ($hotel->approval_status == "Pending L1") {
            Hotel::where("no_htl", $noHtl)->update([
                "approval_status" => "Pending L2",
            ]);

            $managerId = Employee::where("id", $hotel->user_id)->value(
                "manager_l2_id",
            );
            ...
            $employeeName = Employee::where("id", $hotel->user_id)
                ->pluck("fullname")
                ->first();
            ...
            $base64Image =
                "data:image/png;base64," . base64_encode($imageContent);
            $textNotification =
                "requesting a Ticket and waiting for your Approval with the following details :";
            ...
            if ($managerEmail) {
                ...
                try {
                    ...
                    Log::error(
                        "Email Approve Link Hotel tidak terkirim: " .
                            $e->getMessage(),
                    );
                }
            }
    ...
    public function rejectHotelFromLink(
        Request $request,
        $id,
        $manager_id,
        $status,
    ...
    public function hotelEdit($key)
    {
        ...
        if (!$hotel) {
            return redirect()
                ->route("hotel")
                ->with("error", "Hotel transaction not found");
        }
    ...
    public function hotelUpdate(Request $req, $id)
    {
        ...
        foreach ($req->nama_htl as $index => $value) {
            if (!empty($value)) {
                ...
                if (
                    isset($req->hotel_ids[$index]) &&
                    isset($existingHotels[$req->hotel_ids[$index]])
                ) {
        ...
        if ($statusValue !== "Draft") {
            ...
            if ($managerEmail) {
                ...
                try {
                    ...
                    Log::error(
                        "Email Update Hotel tidak terkirim: " .
                            $e->getMessage(),
                    );
                }
            }
        ...
        return redirect("/hotel")->with(
            "success",
            "Hotel request updated successfully",
        );
    }
    ...
    public function hotelDelete($key)
    {
        ...
        return redirect()
            ->intended(route("hotel", absolute: false))
            ->with("success", "All related hotels deleted successfully");
    }
    ...
    public function hotelApproval()
    {
        ...
        $managerL1Names = Employee::whereIn(
            "employee_id",
            $transactions->pluck("manager_l1_id"),
        )->pluck("fullname", "employee_id");
        $managerL2Names = Employee::whereIn(
            "employee_id",
            $transactions->pluck("manager_l2_id"),
        )->pluck("fullname", "employee_id");
    ...
    public function hotelApprovalDetail($key)
    {
        ...
        $hotels = Hotel::where("no_htl", $hotel->no_htl)->get();
        $userId = Hotel::where("no_htl", $hotel->no_htl)
            ->pluck("user_id")
            ->first();
        // dd($userId);
    ...
    public function updateStatusHotel($id, Request $request)
    {
        ...
        if ($statusApproval == "Request Revision") {
            ...
            foreach ($hotels as $hotel) {
                ...
                // Determine the correct layer based on the hotel's approval status BEFORE rejection
                $rejection->layer =
                    $currentApprovalStatus == "Pending L2" ? 2 : 1;
            ...
            return redirect("/hotel/approval")->with(
                "success",
        ...
        if ($hotel->manager_l2_id == "-") {
            ...
            if ($managerEmail) {
                ...
                try {
                    ...
                    Log::error(
                        "Email Update Status Hotel tidak terkirim: " .
                            $e->getMessage(),
                    );
                }
            }
        ...
        foreach ($hotels as $hotel) {
            ...
            if ($hotel->manager_l2_id == "-") {
                ...
                $approval->layer =
                    $hotel->approval_status == "Pending L2" ? 1 : 2;
            }
            $approval->approval_status = $hotel->approval_status;
            $approval->approved_at = now();
            $approval->save();
        ...
        if ($hotel->manager_l2_id == "-") {
            ...
            if ($managerEmail) {
                ...
                try {
                    ...
                    Log::error(
                        "Email Update Status Hotel Admin tidak terkirim: " .
                            $e->getMessage(),
                    );
                }
            }
        ...
        if ($isFilterApplied) {
            ...
            $transactions = Hotel::whereIn("id", $latestHotelIds)
                ...
                ->when($startDate && $endDate, function ($query) use (
                    ...
                    $query->whereRaw("DATE(tgl_masuk_htl) BETWEEN ? AND ?", [
                        $startDate,
                        $endDate,
                    ]);
                });
            ...
            if (!empty($permissionLocations)) {
                $transactions->whereHas("employee", function ($query) use (
                    $permissionLocations,
                ) {
                    $query->whereIn("work_area_code", $permissionLocations);
                });
            ...
            foreach ($transactions as $transaction) {
                ...
                if ($transaction->manager_l1_id) {
                    ...
                    $managerL1 = Employee::where(
                        "employee_id",
                        $transaction->manager_l1_id,
                    )->first();
                    $transaction->manager_l1_name = $managerL1
                        ? $managerL1->fullname
                        : "Unknown";
                } else {
                ...
                if ($transaction->manager_l2_id) {
                    ...
                    $managerL2 = Employee::where(
                        "employee_id",
                        $transaction->manager_l2_id,
                    )->first();
                    $transaction->manager_l2_name = $managerL2
                        ? $managerL2->fullname
                        : "Unknown";
                } else {
                    // If not exists in transaction, get from requestor's (employee) data
    ...
    public function exportHotelAdminExcel(Request $request)
    {
        ...
        return Excel::download(
            new HotelExport($startDate, $endDate),
            "hotel_report.xlsx",
        );
    }
    ...
    public function ticket(Request $request)
    {
        ...
        $query = Tiket::where("user_id", $userId->id)->orderBy(
            "created_at",
            "desc",
        );
    ...
    public function ticketSubmit(Request $req)
    {
        ...
        foreach ($ticketData["noktp_tkt"] as $key => $value) {
            ...
            if (
                !empty($ticketData["noktp_tkt"][$key]) &&
                !empty($ticketData["jk_tkt"][$key]) &&
                !empty($ticketData["np_tkt"][$key])
            ) {
                $tiket = new Tiket();
        ...
        if ($statusValue !== "Draft") {
            ...
            if ($managerEmail) {
                ...
                try {
                    ...
                    Log::error(
                        "Email Submit tiket tidak terkirim: " .
                            $e->getMessage(),
                    );
                }
            }
        ...
        return redirect()
            ...
            ->with(
                "success",
                "The ticket request has been input successfully.",
            );
    }
    ...
    public function approveTicketFromLink($id, $manager_id, $status)
    {
        ...
        if ($ticket->approval_status == "Pending L1") {
            Tiket::where("no_tkt", $noTkt)->update([
                "approval_status" => "Pending L2",
            ]);
            $managerId = Employee::where("id", $ticket->user_id)->value(
                "manager_l2_id",
            );
            $managerEmail = Employee::where("employee_id", $managerId)->value(
                "email",
            ...
            $managerName = Employee::where("employee_id", $managerId)
                ->pluck("fullname")
                ->first();
            $employeeName = Employee::where("id", $ticket->user_id)
                ->pluck("fullname")
                ->first();
            ...
            if ($managerEmail) {
                ...
                try {
                    ...
                    Log::error(
                        "Email Approve Link Tiket tidak terkirim: " .
                            $e->getMessage(),
                    );
                }
            }
        ...
        return redirect()
            ...
            ->with(
                "success",
                "Transaction Approved, Notification will be send to the employee.",
            );
    }
    ...
    public function rejectTicketFromLink(
        Request $request,
        $id,
        $manager_id,
        $status,
        ...
        return redirect()
            ...
            ->with(
                "success",
                "Transaction Rejected, Notification will be send to the employee.",
            );
    }
    ...
    public function ticketUpdate(Request $req, $id)
    {
        ...
        if ($statusValue !== "Draft") {
            $managerId = Employee::where("id", Auth::id())
                ->pluck("manager_l1_id")
                ->first();
            $managerEmail = Employee::where("employee_id", $managerId)
                ->pluck("email")
            ...
            $managerName = Employee::where("employee_id", $managerId)
                ->pluck("fullname")
                ->first();
            $employeeName = Employee::where("id", Auth::id())
                ->pluck("fullname")
                ->first();
            ...
            if ($managerEmail) {
                ...
                try {
                    ...
                    Log::error(
                        "Email Update Tiket tidak terkirim: " .
                            $e->getMessage(),
                    );
                }
            }
        ...
        return redirect()
            ...
            ->with(
                "success",
                "The ticket request has been updated successfully.",
            );
    }
    public function exportExcel(Request $request)
    {
        ...
        $ca_status = $request->input("ca_status");
        // dd($startDate, $endDate, $fromDate, $untilDate, $stat);

        return Excel::download(
            new CashAdvancedExport(
                $startDate,
                $endDate,
                $fromDate,
                $untilDate,
    ...
    public function ticketDelete($key)
    {
        ...
        return redirect()
            ->route("ticket")
            ->with("success", "Tickets has been deleted");
    }
        ...
        $data = [
            ...
            "passengers" => $tickets->map(function ($ticket) {
                return (object) [
                    ...
                    "ket_tkt" => $ticket->ket_tkt,
                    "company_name" =>
                        $ticket->jns_dinas_tkt === "Cuti"
                            ? $ticket->employee->company_name
                            : $ticket->checkcompany->contribution_level ??
                                $ticket->checkcompanybt->checkCompany
                                    ->contribution_level,
    ...
    public function ticketApproval()
    {
        ...
        $tickets = $transactions->filter(function ($ticket) use ($employee) {
            ...
            if (
                $ticket->approval_status == "Pending L1" &&
                $ticket->manager_l1_id == $employee->employee_id
            ) {
                return true;
            } elseif (
    ...
    public function ticketApprovalDetail($key)
    {
        ...
        // Fetch all tickets associated with the same no_sppd for reference
        $tickets = Tiket::where("no_tkt", $ticket->no_tkt)->get();
        $userId = Tiket::where("no_tkt", $ticket->no_tkt)
            ->pluck("user_id")
            ->first();
        // dd($userId);
    ...
    public function updateStatusTicket($id, Request $request)
    {
        ...
        $ticketEmployeeId = Employee::where("id", $ticketUserId)
            ->pluck("employee_id")
            ->first();
        ...
        if ($statusApproval == "Request Revision") {
            ...
            return redirect("/ticket/approval")->with(
                "success",
                "Request get Revision successfully",
            );
        }
            ...
            if ($managerEmail) {
                ...
                if ($ticket->jns_dinas_tkt == "Dinas") {
                    try {
                        ...
                        Log::error(
                            "Email Update Status Tiket Jika Dinas tidak terkirim: " .
                                $e->getMessage(),
                        );
                    }
                } else {
                    try {
                        ...
                        Log::error(
                            "Email Update Status Tiket Jika tidak dinas tidak terkirim: " .
                                $e->getMessage(),
                        );
                    }
                }
        ...
        foreach ($tickets as $ticket) {
            ...
            if ($ticket->manager_l2_id == "-") {
                ...
                $approval->layer =
                    $ticket->approval_status == "Pending L2" ? 1 : 2;
            }
            $approval->approval_status = $ticket->approval_status;
            $approval->approved_at = now();
            $approval->save();
            ...
            if ($managerEmail) {
                ...
                if ($ticket->jns_dinas_tkt == "Dinas") {
                    try {
                        ...
                        Log::error(
                            "Email Update Status Tiket Admin Jika Dinas tidak terkirim: " .
                                $e->getMessage(),
                        );
                    }
                } else {
                    try {
                        ...
                        Log::error(
                            "Email Update Status Tiket Admin Jika Tidak Dinas tidak terkirim: " .
                                $e->getMessage(),
                        );
                    }
                }
    ...
    public function ticketAdmin(Request $request)
    {
        ...
        if ($isFilterApplied) {
            ...
            $transactions = Tiket::whereIn("id", $latestTicketIds)
                ...
                ->when($startDate && $endDate, function ($query) use (
                    ...
                    $query->whereRaw("DATE(tgl_brkt_tkt) BETWEEN ? AND ?", [
                        $startDate,
                        $endDate,
                    ]);
                })
            ...
            $tickets = Tiket::with("businessTrip")->orderBy(
                "created_at",
                "desc",
            );
            ...
            if ($startDate && $endDate) {
                $tickets->whereRaw("DATE(tgl_brkt_tkt) BETWEEN ? AND ?", [
                    $startDate,
                    $endDate,
                ]);
            }
            ...
            foreach ($tiketIds as $tiketId) {
                $ticketApprovals = TiketApproval::where(
                    "tkt_id",
                    $tiketId,
                )->get();
                foreach ($ticketApprovals as $ticketApprove) {
                    $approvalTickets[] = [
                        "tkt_id" => $tiketId,
        ...
        // Return the view with all the data (empty collections if no filters)
    ...
    public function ticketBookingAdmin(Request $req, $id)
    {
        ...
        if ($req->has("action_tkt_book")) {
            ...
            return redirect()
                ->back()
                ->with("success", "Ticket booking updated successfully.");
        }
    }
    ...
    public function exportTicketAdminExcel(Request $request)
    {
        ...
        return Excel::download(
            new TicketExport($startDate, $endDate, $tktType),
            "ticket_report.xlsx",
        );
    }
