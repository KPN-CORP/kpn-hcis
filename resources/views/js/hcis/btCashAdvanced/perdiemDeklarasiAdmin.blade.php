<script>
    ...
    function calculateTotalAmountPlantation(event) {
        let input = event.target; // Get the input field
        let cleanedValue = cleanNumber(input.value); // Parse number
        input.value = formatNumber(cleanedValue); // Format while typing

        // Recalculate total
        let total = 0;
        document.querySelectorAll('input[name="nominal_bt_perdiem[]"]').forEach((el) => {
            total += cleanNumber(el.value);
        });
    ...
    function onNominalChange() {
        if (groupCompany.value === "Plantations") {
            allowanceInput.addEventListener("input", function() {
                this.value = formatNumberPerdiem(cleanNumber(this.value));
            });
        } else {
            calculateTotalNominalBTPerdiem();
        }
