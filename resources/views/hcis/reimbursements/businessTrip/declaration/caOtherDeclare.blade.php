...
@if (!empty($caDetail['detail_lainnya']) && $caDetail['detail_lainnya'][0]['tanggal'] !== null)
    <div id="form-container-lainnya">
        @foreach ($caDetail['detail_lainnya'] as $index => $lainnya)
            <div id="form-container-bt-lainnya-{{ $loop->index + 1 }}" class="p-2 mb-2 bg-light rounded-3">
                <p class="fs-4 text-primary" style="font-weight: bold; ">Other Expenses {{ $loop->index + 1 }}</p>
                <div id="form-container-bt-lainnya-req-{{ $loop->index + 1 }}" class="card-body bg-white rounded-3 p-2 mb-2" style="border-radius: 1%;">
                ...
                <div id="form-container-bt-lainnya-dec-{{ $loop->index + 1 }}" class="card-body bg-white p-2 rounded-3">
                    <p class="fs-5 text-primary" style="font-weight: bold; ">Other Expenses Declaration</p>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-2">  
                                ...
                                <select class="form-control select2" id="type_bt_lainnya_{{ $loop->index + 1 }}" name="type_bt_lainnya[]">  
                                    <option value="">Select Type...</option>
                                    <option value="Airport Tax" {{ isset($lainnya_dec['type']) && $lainnya_dec['type'] == 'Airport Tax' ? 'selected' : '' }}>Airport Tax</option>
                                    <option value="Excess Baggage" {{ isset($lainnya_dec['type']) && $lainnya_dec['type'] == 'Excess Baggage' ? 'selected' : '' }}>Excess Baggage</option>
                                    <option value="Laundry" {{ isset($lainnya_dec['type']) && $lainnya_dec['type'] == 'Laundry' ? 'selected' : '' }}>Laundry</option>
                                    <option value="Mandatory Insurance Fees" {{ isset($lainnya_dec['type']) && $lainnya_dec['type'] == 'Mandatory Insurance Fees' ? 'selected' : '' }}>Mandatory Insurance Fees</option>  
                                    <option value="Parking Fees" {{ isset($lainnya_dec['type']) && $lainnya_dec['type'] == 'Parking Fees' ? 'selected' : '' }}>Parking Fees</option>
                                    <option value="Porter Service" {{ isset($lainnya_dec['type']) && $lainnya_dec['type'] == 'Porter Service' ? 'selected' : '' }}>Porter Service (for company luggage only)</option>
                                    <option value="Toll Fees" {{ isset($lainnya_dec['type']) && $lainnya_dec['type'] == 'Toll Fees' ? 'selected' : '' }}>Toll Fees</option>
                                </select>  
                            </div>
                            <div class="col-md-4 mb-2">
                                <label class="form-label">Amount</label>
...
@elseif (!empty($declareCa['detail_lainnya']) && $declareCa['detail_lainnya'][0]['nominal'] !== null)
    <div id="form-container-lainnya">
        @foreach ($declareCa['detail_lainnya'] as $index => $lainnya_dec)
            @if (!isset($caDetail['detail_lainnya'][$index]))
                <div id="form-container-bt-lainnya-{{ $loop->index + 1 }}" class="card-body p-2 mb-2 rounded-3 bg-light">
                    <p class="fs-4 text-primary" style="font-weight: bold; ">Other Expenses {{ $loop->index + 1 }}</p>
...
@else
    <div id="form-container-lainnya">
        <div id="form-container-bt-lainnya-1" class="card-body p-2 mb-2 rounded-3 bg-light">
