    ...
    <div id="form-container-meals">
        @foreach ($caDetail['detail_meals'] as $index => $meals)
            <div id="form-container-bt-meals-{{ $loop->index + 1 }}" class="p-2 mb-2 bg-light rounded-3">
                ...
                <div id="form-container-bt-meals-req-{{ $loop->index + 1 }}" class="card-body bg-white p-2 mb-2 rounded-3"
                ...
                <div id="form-container-bt-meals-dec-{{ $loop->index + 1 }}" class="card-body bg-white p-2 rounded-3">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-2">
                                ...
                                <input type="date" name="start_bt_meals[]"
                                    ...
                                    value="{{ $meals_dec['start_date'] ?? ($meals_dec['tanggal'] ?? ($meals_dec['tanggal'] ?? '')) }}"
                            ...
                            <div class="col-md-4 mb-2">
                                ...
                                <input type="date" name="end_bt_meals[]" id="end_bt_meals_{{ $loop->index + 1 }}"
                                    class="form-control end-meals"
                                    value="{{ $meals_dec['end_date'] ?? ($meals_dec['tanggal'] ?? '') }}"
                                    placeholder="mm/dd/yyyy"
                                    onchange="calculateTotalDaysPenginapan(document.getElementById('start_bt_meals_{{ $loop->index + 1 }}'), this, document.querySelector('#total_days_bt_meals_{{ $loop->index + 1 }}'))">
                            </div>
                            <div class="col-md-4 mb-2">
                                <label class="form-label">Total Days</label>
                                <div class="input-group">
                    ...
                    @else
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-2">
                                <label class="form-label">Meals End Plan</label>
                                <input type="date" name="end_bt_meals[]" id="end_bt_meals_{{ $loop->index + 1 }}"
                                    class="form-control end-meals" value="" placeholder="mm/dd/yyyy"
                                    onchange="calculateTotalDaysPenginapan(document.getElementById('start_bt_meals_{{ $loop->index + 1 }}'), this, document.querySelector('#total_days_bt_meals_{{ $loop->index + 1 }}'))">
                            </div>
                            <div class="col-md-4 mb-2">
                                <label class="form-label">Total Days</label>
                                <div class="input-group">
                                    <input class="form-control bg-light total-days-meals"
                                        id="total_days_bt_meals_{{ $loop->index + 1 }}" name="total_days_bt_meals[]"
                                        type="number" min="0" value="" readonly>
                                    <div class="input-group-append">
                                        <span class="input-group-text">days</span>
                                    </div>
                                </div>
        ...
        @foreach ($declareCa['detail_meals'] as $index => $meals_dec)
            @if (!isset($caDetail['detail_meals'][$index]))
                <div id="form-container-bt-meals-{{ $loop->index + 1 }}" class="p-2 mb-2 bg-light rounded-3">
                    <p class="fs-4 text-primary" style="font-weight: bold; ">Meals {{ $loop->index + 1 }}</p>
                    <div class="fs-5 bg-white text-primary p-2">
                        <p class="fs-5 text-primary" style="font-weight: bold; ">Meals Declaration</p>
                        <div class="row">
                            <!-- meals Date -->
                            <div class="col-md-4 mb-2">
